...
/*
TO DO:
1. unit test bullet_map
2. create LoadBullet function
*/
...
class bullet_map
{
   struct pair
    ...
    // insert a key-value pair, note that overlapping key is not possible
    bool insert(int key, bullet buffer_bullet, int lifespan = 1000)
    {
        bool duplicated = false;
        for(int n = 0; n < container.size(); n++)
        {
    ...
    void age_cache(int time_ms)
    {
        ...
        // reduce lifespan and age bullet
        for(int n = 0; n < container.size(); n++)
        {
            container[n].lifespan_in_miliseconds -= time_ms;
...
bool LoadBullet(int Key, bullet_map &BulletCache, int X = 100, int Y = 100, int Speed = 1, int Direction = 135, int Lifespan = 1000)
{
    bullet bullet_buffer(X, Y, Speed, Direction);
    BulletCache.insert(Key, bullet_buffer, Lifespan);
    return true;
}
...
int main(int argc, char* args[])
{
    ...
    if(IsInitialized)        // Start The Game
    {
        ...
        LoadSprite(TextureCache[0], SpriteCache, 1, 1000);
        LoadBullet(1, BulletCache, 300, 300, 1, 135, 3000);
        ...
        while(true)
        {
            ...
            SpriteCache.find(1)->Sprite.DestinationRectangle.x = BulletCache.find(1)->Bullet.GetX();
            SpriteCache.find(1)->Sprite.DestinationRectangle.y = BulletCache.find(1)->Bullet.GetY();
