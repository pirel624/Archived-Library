...
/*
TO DO:
1. implement precise collision detection (use float)
*/
...
int main(int argc, char* args[])
{
    ...
    if(IsInitialized)        // Start The Game
    {
        ...
        SpriteCache.load_sprite(TextureCache[0], 1, 2, 1000000000);
        HitboxCache.load_hitbox(1, 2, 1000000000);
        HitboxCache.find(1, 2)->Hitbox.MoldAgainstSprite(SpriteCache.find(1, 1)->Sprite);
        BulletCache.load_bullet(1, 2, 1000000000, 300, 300, 0.1f, 315);
        ...
        while(true)
        {
            ...
            HitboxCache.age_cache(10);

            if(KeyboardInput.Pressed_W){HitboxCache.find(1, 1)->Hitbox.MoveY(-10);}
            if(KeyboardInput.Pressed_A){HitboxCache.find(1, 1)->Hitbox.MoveX(-10);}
            if(KeyboardInput.Pressed_S){HitboxCache.find(1, 1)->Hitbox.MoveY(10);}
            if(KeyboardInput.Pressed_D){HitboxCache.find(1, 1)->Hitbox.MoveX(10);}
            ...
            HitboxCache.find(1, 2)->Hitbox.SetCenterX(BulletCache.find(1, 2)->Bullet.GetX());
            HitboxCache.find(1, 2)->Hitbox.SetCenterY(BulletCache.find(1, 2)->Bullet.GetY());
            SpriteCache.find(1, 2)->Sprite.DestinationRectangle.x = HitboxCache.find(1, 2)->Hitbox.GetSDLRectX();
            SpriteCache.find(1, 2)->Sprite.DestinationRectangle.y = HitboxCache.find(1, 2)->Hitbox.GetSDLRectY();
            
            if(hitbox::IsColliding(HitboxCache.find(1, 1)->Hitbox, HitboxCache.find(1, 2)->Hitbox))
            {
                std::cout << "Collision\n";
                break;
            }
