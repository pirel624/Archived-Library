...
{
    private:
    SDL_Rect Rectangular_Bound;
    char Mode = 'C';           // C for Circular, R for Rectangular

    public:
    void SetWidth(int value){Rectangular_Bound.w = value;}
    void SetHeight(int value){Rectangular_Bound.h = value;}
    void SetCenterX(int Value)
    {
        Rectangular_Bound.x = Value - (Rectangular_Bound.w / 2);
    }
    void SetCenterY(int Value)
    {
        Rectangular_Bound.y = Value - (Rectangular_Bound.h / 2);
    }
    ...
    int GetCenterX()
    {
        return (Rectangular_Bound.x + (Rectangular_Bound.w / 2));
    }
    int GetCenterY()
    {
        return (Rectangular_Bound.y + (Rectangular_Bound.h / 2));
    }

    hitbox(char mode = 'C', int CenterX = 100, int CenterY = 100, int Width = 30, int Height = 30)
    {
        Mode = mode;
        SetCenterX(CenterX);
        SetCenterY(CenterY);
    ...
    {
        return Rectangular_Bound;
    }

    static bool is_colliding(hitbox shape1, hitbox shape2)
    {
        if(shape1.GetMode() == 'C' && shape2.GetMode() == 'C')
        {
            ...
            std::sqrt
            (
                (shape2.GetCenterX() - shape1.GetCenterX()) *
                (shape2.GetCenterX() - shape1.GetCenterX()) +
                (shape2.GetCenterY() - shape1.GetCenterY()) *
                (shape2.GetCenterY() - shape1.GetCenterY())
            );
            return (distance <= (shape1.GetBoundingBox().w + shape2.GetBoundingBox().w));
        }
...
class sprite_map
{
    ...
    {
        printf("Bootsrapping %d %d succeeded\n", key, Dimension);
        ...
        printf("Buffering %d %d succeeded\n", key, Dimension);
...
int main(int argc, char* args[])
{
    ...
    if(IsInitialized)        // Start The Game
    {
        ...
        SpriteCache.load_sprite(TextureCache[0], 1, 1, 1000000);
        SpriteCache.load_sprite(TextureCache[0], 2, 1, 1000000);
        BulletCache.load_bullet(1, 1, 100, 100, 0.1f, 315, 1000000);
        BulletCache.load_bullet(1, 1, 100, 100, 1, 90, 10000);
        std::cout << "We Got Here !!" << std::endl;

        SpriteCache.find(1, 1)->Sprite.DestinationRectangle.x = 300;
        SpriteCache.find(1, 1)->Sprite.DestinationRectangle.y = 300;
        SpriteCache.load_sprite(TextureCache[0], 1, 1, 1000);

        //SpriteCache.erase(2, 1);
        //BulletCache.erase(1, 1);
        ...
        while(true)
        {
            ...
            BulletCache.age_cache(10);
            SpriteCache.age_cache(10);
            SpriteCache.find(1, 1)->Sprite.DestinationRectangle.x = BulletCache.find(1, 1)->Bullet.GetX();
            SpriteCache.find(1, 1)->Sprite.DestinationRectangle.y = BulletCache.find(1, 1)->Bullet.GetY();
